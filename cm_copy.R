...
###########
lagged<-readRDS(file = "laggeddata.rds")
lagged[,3]=NULL
View(lagged)

decemberData <- lagged[which(lagged$Date=="2015-12-31"),]
decemberData[,1]=NULL
View(decemberData)
##Categorical Variables
decemberData$Industry=as.factor(decemberData$Industry)
contrasts(decemberData$Industry) = contr.treatment(213)

##Linear Regression
y<-decemberData$RETMONTH
decemberLinear<-lm(y ~ RETMONTH_SPX + Close + Adj_Close + Volume  + 
                     Adj_Volume + Industry +  MARKETCAP +  REVENUEUSD + COR +GP + RND+SGNA + OPEX + OPINC + 
                     EBITUSD + INTEXP + TAXEXP +	CONSOLINC +	NETINCNCI	+ NETINC + PREFDIVIS+ NETINCCMNUSD +	
                     EPSUSD	+ SHARESWA +	DPS + 	DEPAMOR +	SBCOMP + CAPEX + NCF + ASSETS + CASHNEQUSD + 
                     INVENTORY + LIABILITIES + DEBTUSD + INVESTMENTS + EQUITY + BM + SHARESBAS + SHAREFACTOR + 
                     ...
                     FCFPS + GROSSMARGIN + NETMARGIN	+ PE + PS	+ PB +ROIC + SPS +PAYOUTRATIO	+ ROA	+ ROE	+ ROS,
                   data = decemberData)
summary(decemberLinear)
decemberMSE<-mean(decemberLinear$residuals^2)
mse<-function(regression)
{
  m<-mean(regression$residuals^2)
  return(m)
}
mse(decemberLinear)
x = model.matrix(y~ RETMONTH_SPX + Close + Adj_Close + Volume  + 
                   Adj_Volume + Industry +  MARKETCAP +  REVENUEUSD + COR +GP + RND+SGNA + OPEX + OPINC + 
                   EBITUSD + INTEXP + TAXEXP +	CONSOLINC +	NETINCNCI	+ NETINC + PREFDIVIS+ NETINCCMNUSD +	
                   EPSUSD	+ SHARESWA +	DPS + 	DEPAMOR +	SBCOMP + CAPEX + NCF + ASSETS + CASHNEQUSD + 
                   INVENTORY + LIABILITIES + DEBTUSD + INVESTMENTS + EQUITY + BM + SHARESBAS + SHAREFACTOR + 
...
lassoDecember<-cv.glmnet(x, RETMONTH,alpha=1)
mse(lassoDecember)
plot(lassoDecember)
